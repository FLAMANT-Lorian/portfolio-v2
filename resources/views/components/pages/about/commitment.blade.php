...
<section>
    <div class="screen-width px-default pt-default grid-default gap-y-14">
        <div class="col-span-full rl:col-span-5">
            <h2 data-reveal
                data-dir="top"
                class="text-h2 text-black font-medium col-span-full pb-3">
                {{ $content['title'] }}
            </h2>
            <p data-reveal
               data-dir="right"
               data-delay="0.15"
               class="text-p text-gray-dark">
                {{ $content['text'] }}
            </p>
        ...
        <ul class="col-span-full rl:col-span-6 rl:col-start-7 grid md:grid-cols-2 rl:grid-cols-2 gap-6">
            @php
                $delay = 0;
            @endphp
            @foreach($content['list'] as $id =>  $commitment)
                @php
                    if ($id === 3) {
                        $delay = 0;
                    }
                ...
                <li data-reveal
                    data-dir="top"
                    data-delay="{{ $delay }}"
                    class="relative before:absolute before:top-0 before:left-0 hover:before:w-10 hover:before:h-10 before:ease-all flex flex-col gap-3 p-4 rl:p-6 before:content-[''] before:w-6 before:h-6 before:border-l-4 before:border-t-4 before:border-red">
